"""
OpenMind Telegram Bot — Free AI for Everyone
...
class RateLimiter:
    ...
    def is_allowed(self, user_id: int) -> bool:
        ...
        self.user_timestamps[user_id] = [
            ts for ts in self.user_timestamps[user_id] if ts > window
        ]
...
SYSTEM_PROMPT = """You are OpenMind AI — a free, open-source AI assistant created by the OpenMind community.
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = (
        ...
        "I'm a free, open-source AI assistant powered by cutting-edge open models.\n\n"
        "**Quick Start:**\n"
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        "• `/help` — This message\n\n"
        "**Rate Limit:** {rate} messages/min per user\n"
        "**Current Model:** `{model}`"
    ).format(rate=RATE_LIMIT, model=DEFAULT_MODEL)
...
async def cmd_models(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for name, model_id in AVAILABLE_MODELS.items():
        ...
        keyboard.append([InlineKeyboardButton(
            f"{name}{indicator}", callback_data=f"setmodel:{model_id}"
        )])
...
async def cmd_model(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(f"Current model: `{DEFAULT_MODEL}`", parse_mode=ParseMode.MARKDOWN)
        return

    model_name = context.args[0].lower()
    if model_name in AVAILABLE_MODELS:
        DEFAULT_MODEL = AVAILABLE_MODELS[model_name]
        await update.message.reply_text(f"✅ Model set to **{model_name}** (`{DEFAULT_MODEL}`)", parse_mode=ParseMode.MARKDOWN)
    elif model_name in AVAILABLE_MODELS.values():
        DEFAULT_MODEL = model_name
        await update.message.reply_text(f"✅ Model set to `{DEFAULT_MODEL}`", parse_mode=ParseMode.MARKDOWN)
    else:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE, explicit_msg: str = None):
